// Accesso App Internal API. 0.6.0
// ---
...
export const accountEditBadRequest = typed.object({
  error: typed.union('invalid_payload', 'invalid_form'),
});

/* User not authorized */
export const accountEditUnauthorized = typed.nul;

/* Something went wrong */
export const accountEditInternalServerError = typed.nul;
export type AccountEditFail =
  | {
      status: 'bad_request';
      error: typed.Get<typeof accountEditBadRequest>;
    }
  | {
      status: 'unauthorized';
      error: typed.Get<typeof accountEditUnauthorized>;
    }
  | {
      status: 'internal_server_error';
      error: typed.Get<typeof accountEditInternalServerError>;
    }
  | GenericErrors;

/* Edit description details of the account */
export const accountEdit = createEffect<AccountEdit, AccountEditDone, AccountEditFail>({
  async handler({ body }) {
    const name = 'accountEdit.body';
    const response = await requestFx({
      path: '/account.edit',
      method: 'POST',
      body,
    ...
    return parseByStatus(name, response, {
      200: ['ok', accountEditOk],
      400: ['bad_request', accountEditBadRequest],
      401: ['unauthorized', accountEditUnauthorized],
      500: ['internal_server_error', accountEditInternalServerError],
    });
  },
});
//#endregion accountEdit
...
export const applicationGetOk = typed.object({
  /* Application which has accesso registration possibilities */
  application: typed.object({
    id: typed.string,
    title: typed.string,
...
export const applicationGetBadRequest = typed.object({
  error: typed.union('not_found'),
});

/* Something went wrong */
export const applicationGetInternalServerError = typed.nul;
export type ApplicationGetFail =
  | {
      status: 'bad_request';
      error: typed.Get<typeof applicationGetBadRequest>;
    }
  | {
      status: 'internal_server_error';
      error: typed.Get<typeof applicationGetInternalServerError>;
    }
  | GenericErrors;

/* Get application info from id */
export const applicationGet = createEffect<ApplicationGet, ApplicationGetDone, ApplicationGetFail>({
  async handler({ body }) {
    const name = 'applicationGet.body';
    const response = await requestFx({
    ...
    return parseByStatus(name, response, {
      200: ['ok', applicationGetOk],
      400: ['bad_request', applicationGetBadRequest],
      500: ['internal_server_error', applicationGetInternalServerError],
    });
  },
});
